...
const request = async <Response>({
  ...
  url: string;
  options?: CustomOptions;
  // Sửa chữ ký hàm để trả về Promise có kiểu dữ liệu đúng
  ...
  try {
        ...
        if (isClient) {
          if (!clientLogoutRequest) {
            clientLogoutRequest = fetch("/api/auth/logout", {
              method: "POST",
              body: null,
              // Sử dụng HeadersInit thay vì any
              headers: {
                ...baseHeaders
              } as HeadersInit
            });
          ...
          const accessToken = 
          (options?.headers as Record<string, string>)?.Authorization?.split("Bearer ")[1] ?? "";
          redirect(accessToken ? `/logout?token=${accessToken}` : "/logout");
        }
      } else {
        throw new HttpError({ status: res.status, message: (payload as { message: string }).message });
      }
    }
    ...
    if (isClient) {
      const normalizedUrl = normalizePath(url);
      if (normalizedUrl === 'api/auth/login') {
        const token = (payload as LoginSuccessPayload)?.token; 
...
const http = {
  ...
  // },
  post<Response, Body>({ url, body, options }: { url: string; body: Body; options?: Omit<CustomOptions, "body"> }) {
    return request<Response>({ method: "POST", url, options: { ...options, body } });
  },

  put<Response, Body>({ url, body, options }: { url: string; body: Body; options?: Omit<CustomOptions, "body"> }) {
